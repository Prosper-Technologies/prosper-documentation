...
```
POST /v1/campaigns/{campaign_id}/targets
```
...
| **2** | Add targets and execute calls using `POST /v1/campaigns/{campaign_id}/targets` |
| **4** | Receive results via the campaign’s webhook                                     |
| **5** | Retrieve the target details using `GET /v1/targets/{target_id} `               |
